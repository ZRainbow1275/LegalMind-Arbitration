// dev/src/app/api/documents/[id]/preview/route.ts
// 文档预览：用于 Prototype 文档节点 previewUrl
  ...
  try {
    ...
    const url = await getSignedUrl(
      ...
      new GetObjectCommand({
        Bucket: bucket,
        Key: key,
        ResponseContentDisposition: buildContentDispositionInline(document.originalName),
    ...
    await AuditLogger.log({
      ...
      resource: 'documents',
      action: 'preview',
      details: { documentId: document.id, caseId: document.caseId, bucket, key },
      result: 'SUCCESS',
    });

    return NextResponse.redirect(url, { status: 302 });
  } catch (error) {
    logger.error({ err: error }, '文档预览失败');
